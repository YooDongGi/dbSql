...
FROM emp;
--분석함수를 사용하지 않고 구하기.
SELECT e.empno, e.ename, e.sal, a.deptno, a.max_sal
FROM emp e,
...
FROM emp;
--분석함수를 사용하지 않고 구하기
SELECT e.empno, e.ename, e.sal, a.deptno, a.min_sal
FROM emp e,
...
이전/이후 n행 값 가져오기
LAG(col, [건널띌 행 수 - default 1], [값이 없을 경우 적용할 기본값])
SELECT empno, ename, hiredate, job, sal,
    LAG(sal,3, 0) OVER (ORDER BY sal DESC , hiredate) lag_sal
FROM emp;
...
windowing에서 사용할 수 있는 특수 키워드
 ...
 - CURRENT ROW 현재 행
 - UNBOUNDED FOLLOWING 현재 행 기준 모든 이후 행
 - n PRECEDING(n은 정수) 현재 행 기준 이전 n행
 - n FOLLOWING(n은 정수) 현재 행 기준 이후 n행

현재 행 이전의 모든 행 ~ 현재 ==> 행들을 정렬할 수 있는 기준이 있어야한다.
SELECT empno, ename, sal,
                        SUM(sal) OVER(ORDER BY sal, hiredate 
                                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) c_sum
                                  --  ROWS UNBOUNDED PRECEDING) (동일한 표현)
FROM emp;

선행하는 이전 첫번째 행 ~ 후행하는 이후 첫번째 행까지
...
SELECT empno, ename, deptno, sal,
        ...
        SUM(sal) OVER(ORDER BY sal RANGE UNBOUNDED PRECEDING) range_sum1,
        SUM(sal) OVER(ORDER BY sal) c_sum
FROM emp;


DBMS 입장에서 SQL 처리 순서 
1. 요청한 SQL과 동일한 SQL이 이미 실행된 적이 있는지 확인하여 실행된 적이 있다면 SHARED POOL에 저장된 실행계획을 재활용한다.
    1-2. 만약 SHARED POOL에 저장된 실행계획이 없다면 (동일한 SQL이 실행된 적이 없음) 실행계획을 세운다
*** 동일한 SQL 이란?
    - 결과만 같다고 동일한 SQL이 아님
    - DBMS 입장에서는 완벽하게 문자열이 동일해야 동일한 SQL
...
LEVEL 2 : REPEATABLE READ (반복적 읽기)
          ...
          신규(INSERT)로 입력하는 데이터에 대해서는 선행 트랜잭션에 영향을 미친다.
          ==> Phantom Read(귀신 읽기)
              존재하지 않았던 데이터가 갑자기 조회되는 현상
LEVEL 3 : SERIALIZABLE READ (직렬화 읽기)
          후행 트랜잭션의 작업이 선행 트랜잭션에 아무런 영향을 주지 않는 단계
          *** DBMS마다 LOCKING 메커니즘이 다르기 때문에 ISOLATION LEVEL을 함부로 수정하는 것은 위험하다.
